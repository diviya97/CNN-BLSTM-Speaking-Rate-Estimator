...
def mix_timit_and_swbd_data(X_timit, Y_timit, X_swbd, Y_swbd):
    ...
    for fold in range(n):
        x = X_timit[fold]
        x.extend(X_swbd[fold])
        y = Y_timit[fold]
        y=np.append(y, Y_swbd[fold])
...
def load_data_timit():
    ...
    file_names = file1['timitSpurtName']
...
X_timit, Y_timit = load_data_timit()
X_swbd, Y_swbd = load_data_swbd()
X, Y = mix_timit_and_swbd_data(X_timit, Y_timit, X_swbd, Y_swbd)
